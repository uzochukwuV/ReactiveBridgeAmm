...
contract ReactiveBridge is AbstractCallback {
    ...
    mapping(uint256 => mapping(address => address))
        public originToCanonicalAddress;
    ...
    constructor(
        ...
        oracleRegistry = ITokenOracleRegistry(
            0x8Bd2038bc18c9156a6427bf704dEF80953Fb8Aca
        );
        supportedChains[11155111] = true;
        supportedTokens[0x1c7D4B196Cb0C7B01d743Fbc6116a902379C7238] = true;
        chainToTokens[11155111][
    ...
    function test() external {
        emit Received(REACTIVE_SUBSCRIPTION_ID, block.chainid, 0.001 ether);
    }
    ...
    function placeOrder(
        ...
        require(
            chainToTokens[destinationChain][tokenRequested],
            "UNSUPPORTED_TOKEN_REQUESTED"
        );
        require(amountDeposited > 0 && minFillAmount > 0, "ZERO_AMOUNT");

        if (usePriceFeed) {
            require(
                oracleRegistry.tokenFeeds(tokenDeposited) != address(0),
                "NO_FEED_DEPOSITED"
            );
            require(
                oracleRegistry.tokenFeeds(tokenRequested) != address(0),
                "NO_FEED_REQUESTED"
            );
        }
    ...
    function callbackReceiveOrder(
        ...
        require(
            destinationOrders[originChain][orderId].orderer == address(0),
            "ORDER_EXISTS"
        );
    ...
    function fillOrder(
        ...
        require(
            amountRequestedToFill >= oConfig.minFillAmount,
            "BELOW_MIN_FILL"
        );
        require(
            block.timestamp <= order.timestamp + oConfig.duration,
            "ORDER_EXPIRED"
        );
        // check if amount of requested token remaing is greater than amount to fill
        require(
            tConfig.amountRequestedRemaining >= amountRequestedToFill,
            "INSUFFICIENT_AMOUNT"
        );
        ...
        if (tConfig.tokenRequested == address(0)) {
            ...
            require(
                IERC20(tConfig.tokenRequested).transferFrom(
                    msg.sender,
                    address(this),
    ...
    function callbackCompleteFill(
        ...
        require(order.isCancelled != true, "ORDER_CANCELLED");
        ...
        if (oConfig.usePriceFeed) {
            ...
            if (
                tConfig.tokenDeposited != address(0) &&
                tConfig.tokenRequested != address(0)
            ) {
                baseAmountToSend = oracleRegistry.convertAmount(
                    ...
                    amountFilled,
                    ERC20Decimals(
                        originToCanonicalAddress[order.originChain][
                            tConfig.tokenRequested
                        ]
                    ).decimals(),
            ...
            uint256 expectedAmount = (tConfig.amountDeposited * amountFilled) /
                tConfig.amountRequested;
            uint256 slippageTolerance = (expectedAmount * oConfig.slippage) /
                10000;
            ...
            uint256 premiumAmount = (baseAmountToSend * oConfig.premiumBps) /
                10000;
            amountToSend = baseAmountToSend + premiumAmount;
        } else {
            // Fixed ratio
            uint256 baseRatio = (tConfig.amountDeposited * amountFilled) /
                tConfig.amountRequested;
        ...
        require(
            ledger[order.orderer][tConfig.tokenDeposited] >= amountToSend,
            "INSUFFICIENT_LEDGER"
        );
        ...
        address fillerAddress = filler;
        if (tConfig.tokenDeposited == address(0)) {
            payable(fillerAddress).transfer(finalAmountToSend);
        } else {
            require(
                IERC20(tConfig.tokenDeposited).transfer(
                    fillerAddress,
                    finalAmountToSend
                ),
                "TRANSFER_FAILED"
        ...
        originOrders[order.orderId].tokenConfig = abi.encode(
            tConfig.tokenDeposited,
            tConfig.tokenRequested,
            tConfig.amountDeposited,
            tConfig.amountRequested,
    ...
    function callbackFinalizeOrder(
        ...
        uint256 protocolFee = (fill.amountFilled * feeConfig.protocolFeeBps) /
            10000;
        ...
        if (tConfig.tokenRequested == address(0)) {
            ...
            require(
                IERC20(tConfig.tokenRequested).transfer(
                    order.orderer,
                    amountL1OrderwillRecieve
                ),
                "TRANSFER_FAILED"
            );
        }
    ...
    function markOrderAsCanceledCallbackL2(
        address sender,
        uint256 orderId,
        uint256 originChain,
        uint256 destinationChain,
    ...
    function markOrderAsCanceledCallbackL1(
        ...
        require(
            tConfig.amountDepositedRemaining <=
                ledger[order.orderer][tConfig.tokenDeposited],
            "LEDGER_DOES_NOT_MATCH"
        );

        ledger[order.orderer][tConfig.tokenDeposited] -= tConfig
            .amountDepositedRemaining;
        if (tConfig.tokenDeposited == address(0)) {
    ...
    function cancelExpiredOrder(uint256 orderId) external {
        ...
        require(
            block.timestamp >
                order.timestamp + oConfig.duration + CROSS_CHAIN_BUFFER,
            "NOT_EXPIRED_YET"
        );
    ...
    function testOracle(
        ...
        return
            oracleRegistry.convertAmount(
                tokenA,
                tokenB,
                amountA,
                ERC20Decimals(tokenA).decimals(),
        ...
        )
    {
        ...
        if (oConfig.usePriceFeed) {
            ...
            baseCost =
                (tConfig.amountDeposited * amountToFill) /
                tConfig.amountRequested;
        }
    ...
    function withdrawProtocolFees(address token) external onlyOwner {
        ...
        if (token == address(0)) {
            ...
            require(balance > 0, "NO_FEES");
            require(
                IERC20(token).transfer(feeConfig.feeRecipient, balance),
                "TRANSFER_FAILED"
            );
        }
    }
